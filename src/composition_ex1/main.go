...
func (s *sysadmin) administrate(system string) {
	fmt.Printf("Name: %s, System: %s", s.name, system)
}
...
func (p *programmer) develop(system string) {
	fmt.Printf("Name: %s, System: %s", p.name, system)
}
...
func main() {
	...
	admins.pushAdmin(&sysadmin{name: "Ruth"})

	// Push two new programmers onto devs.
	devs.pushDev(&programmer{name: "Saul"})
	devs.pushDev(&programmer{name: "Merna"})
	...
	techfirm := company{administrator: admins.popAdmin(), developer: devs.popDev()}
